...
redisClient.on('connect', () => {
  console.log('[CreatorTracker] Connected to Redis for session storage');
});

redisClient.on('error', (err) => {
  console.error('[CreatorTracker] Redis connection error:', err);
});

redisClient.on('close', () => {
  console.log('[CreatorTracker] Redis connection closed');
});
