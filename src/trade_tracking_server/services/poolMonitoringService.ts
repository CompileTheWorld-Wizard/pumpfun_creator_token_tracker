...
import { convertEventToTrackerFormat } from './eventConverter.js';
import { ParsedEvent, ParsedTransaction } from './type.js';
import { extractBondingCurveForEvent } from './utils/transactionUtils.js';
...
class LiquidityPoolMonitor {
  ...
  async stopMonitoring(sessionKey: string): Promise<void> {
    const session = this.sessions.get(sessionKey);
    if (!session) {
      return;
    }
  ...
  private calculatePeakPricesFromTimeseries(
    ...
    const firstSellDataPoint : TimeseriesDataPoint | undefined = session.firstSellTxId 
...
export class PoolMonitoringService {
  ...
  async startMonitoring(
    ...
    return this.monitor.startMonitoring(walletAddress, tokenAddress, poolAddress, maxDuration, initialPriceData, createdAt, firstBuyTxId);
  }
