...
interface TokenCreatorInfo {
  ...
  devBuyTimestamp?: number;
  devBuyBlockNumber?: number;
}
...
export class TokenService {
  ...
  async refreshSkipTokensCache(): Promise<void> {
    try {
      const { dbService } = await import('../database/index.js');
  ...
  async getTokenActivities(mintAddress: string, isRetry: boolean = false): Promise<SolscanActivitiesResponse | null> {
    try {
      ...
      const data = await response.json() as SolscanActivitiesResponse;
  ...
  async extractFirstBuyFromActivities(
    ...
    const devBuyTimestamp = firstSwap.block_time ? firstSwap.block_time * 1000 : undefined; // Convert to milliseconds
    const devBuyBlockNumber = firstSwap.block_id;
    
    console.log(`  📅 Dev Buy Timestamp: ${devBuyTimestamp ? new Date(devBuyTimestamp).toISOString() : 'N/A'}`);
