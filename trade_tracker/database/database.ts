...
interface TokenData {
  ...
  dev_buy_timestamp?: number;
  dev_buy_block_number?: number;
  token_name?: string;
  symbol?: string;
  image?: string;
  twitter?: string;
...
class DatabaseService {
  ...
  /**
   * Get the current password hash from passwords table (shared with creator_tracking_server)
   */
  ...
  /**
   * Update password hash in passwords table (shared with creator_tracking_server)
   */
  async updatePasswordHash(passwordHash: string): Promise<void> {
    try {
      // Insert new password (creator_tracking_server uses INSERT, not UPDATE)
      const insertQuery = `
