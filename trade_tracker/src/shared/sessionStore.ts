...
redisClient.on('connect', () => {
  console.log('[TradeTracker] Connected to Redis for session storage');
});

redisClient.on('error', (err) => {
  console.error('[TradeTracker] Redis connection error:', err);
});

redisClient.on('close', () => {
  console.log('[TradeTracker] Redis connection closed');
});
