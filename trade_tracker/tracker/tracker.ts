...
import { PoolMonitoringService } from "../services/poolMonitoringService.js";
import { StreamerService } from "../services/streamerService.js";
import { convertEventToTrackerFormat } from "../services/eventConverter.js";
import { ParsedEvent, ParsedTransaction } from "../services/type.js";
import { extractBondingCurveForEvent } from "../services/utils/transactionUtils.js";
import { redisService } from "../services/redisService.js";

dotenv.config();

interface ShyftAllTokensForWalletAPIResponse {
  success: boolean,
  message: string,
  result: Array<{
    address: string,
...
interface SolscanAccountDefiActivitiesAPIResponse {
  success: boolean,
  data: Array<{
    block_id: number,
    trans_id: string,
...
class TransactionTracker {
    ...
    try {
      ...
      if (txType === 'BUY' || txType === 'SELL') {
        ...
        if (tokenPriceSol === null || isNaN(tokenPriceSol)) {
          ...
          await this.processWalletTracking(result, tokenAddress, txType, null);
          return;
        }
  ...
  private async fetchAndSaveCreatorTokenCount(creatorAddress: string, tokenAddress: string): Promise<void> {
    try {
      ...
      while (hasMore) {
        ...
        const data = await response.json() as SolscanAccountDefiActivitiesAPIResponse;
  ...
  async analyzeWallet(walletAddress: string): Promise<any> {
    try {
      ...
      const data = await response.json() as ShyftAllTokensForWalletAPIResponse;
